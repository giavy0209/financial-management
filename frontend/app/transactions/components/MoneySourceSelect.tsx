...
export default function MoneySourceSelect({ value, onChange }: MoneySourceSelectProps) {
  ...
  const { moneySources, pagination, loading } = useSelector((state: RootState) => state.moneySource)
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        {selectedMoneySource ? selectedMoneySource.name : "Select a money source"}
      </button>
          ...
          {moneySources.map((moneySource: MoneySourceFieldsFragment) => (
            <div
              ...
              className={`cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-indigo-600 hover:text-white ${
                value === moneySource.id ? "bg-indigo-600 text-white" : "text-gray-900"
              }`}
              onClick={() => {
